...
REC_MODEL_PATH = "data/rec_model.h5"
RATING_PATH = 'data/rating.ann'
...
class EnsembleRecommender():
    ...
    def __init__(self, rating_df: pd.DataFrame, movie_df: pd.DataFrame, rating_matrix: pd.DataFrame, item_vector: np.ndarray, save_path: str = RATING_PATH):
        """
    ...
    def NCF_recommendation(self, userId: int, top_k: int = 20):
        """
        ...
        d = {'user_encoded': [
            client_encoded] * len(movie_poll_encoded), 'movie_encoded': movie_poll_encoded}
        ...
        ratings = model.predict(
            [client_df['user_encoded'], client_df['movie_encoded']])
        ...
        recommend_movieId = [self.movie_encoded2movie.get(
            movie_poll_encoded[x]) for x in top_ratings_idx]

        # format the output for better user experience
        top_movie_rec = pd.DataFrame(
            {'movieId': recommend_movieId, 'prediction': top_ratings}).set_index('movieId')
    ...
    def get_rated_movies(self, userId, threshold=3):
        ...
        high_rates = all_rates[all_rates['rating']
                               >= threshold]['rating'].values
        high_rate_movie = all_rates[all_rates['rating']
                                    >= threshold]['movieId'].values
    ...
    def ANN_recommendation(self, userId, dimension=14, metric='angular',
        ...
        u.load(self.save_path)

        # construct the recommendation for the user
        high_rate_movie, rate = self.get_rated_movies(
        ...
        if len(high_rate_movie) >= 1:
            ...
            for movieid in high_rate_movie:
                movie, dist = u.get_nns_by_item(
                    movieid, top_n, include_distances=True)
                movielist.extend(movie[1:])

                # get the weighted distance based on rating scores
                weighted_dist = (
                    np.array(dist[1:])/rate[np.where(high_rate_movie == movieid)]).tolist()
            ...
            if len(movielist) > 20:
                sorted_recommend = np.array(
                    movielist)[np.array(distancelist).argsort()]
                movielist = sorted_recommend[:20]

        # construct a dataframe for final output
        top_movie_rec = self.movie_df.loc[self.movie_df['movieId'].isin(
            movielist)].set_index('movieId')
    ...
    def Popular_recommendation(self, top_k=20):
        ...
        df_grouped.columns = np.array(['count'])

        df_group_avg = pd.DataFrame(grouped_rating)
        df_group_avg.columns = np.array(['avg_rating'])
        ...
        top_k_rec = df_grouped.loc[df_grouped['count'] >
                                   1000].loc[df_grouped['avg_rating'] > 4.0][:top_k]

        # construct a dataframe for final output
        top_movie_rec = self.movie_df.loc[self.movie_df['movieId'].isin(
            top_k_rec.index.values)].set_index('movieId')
    ...
    def User_Classification(self, userId):
        ...
        if userId not in self.user_ids:
            ...
            num_of_rated_movies = len(
                self.rating_df.loc[self.rating_df.userId == userId]['movieId'].unique())
